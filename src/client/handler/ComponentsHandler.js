            ...
            for (const file of readdirSync('./src/components/' + directory).filter((f) => f.endsWith('.js'))) {
                try {
                    ...
                    if (module.__type__ === 3) {
                        ...
                        switch (module.type) {
                            case 'modal': {
                                this.client.collection.components.modals.set(file, module);
                                break;
                            }
                            case 'select': {
                                this.client.collection.components.selects.set(file, module);
                                break;
                            }
                            case 'button': {
                                this.client.collection.components.buttons.set(file, module);
                                break;
                            }
                            default: {
